...
- **Date-group select / deselect all** — every device · date group header has a checkbox that selects every visible card in that group in one click. Clicking it again (or when all cards are already selected) deselects them. The checkbox shows an indeterminate state when only some cards in the group are selected.
